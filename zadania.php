...
<?php
    ...
    $q = "SELECT id, login, haslo FROM pracownicy";
    ...
    while($row = mysqli_fetch_assoc($result))
    {
        ...
        {
            $logowanie= "true";
//zapisanie do zmiennej id pracownika, ktorego udalo się zalogować
            $_SESSION["pracownik"]=$row['id'];   
        }
    }
                ...
                <div class="row">
                    <div class="col-md-12">
                        <div class="sekcja1_srodek">
                            <center>
                                <h3>Zadania na dziś</h3><br> 
                            </center>
                            <?php
                            //pobieranie dnia miesiąca
                                $t=date('d-m-Y');
                                $day = strtolower(date("D",strtotime($t)));
                            //przypisywanie aktualnie zalogowanego pracownika id do zmiennej lokalnej
                                $pracownik_aktualny = $_SESSION["pracownik"];
        
                                $conn = mysqli_connect("localhost", "projekt_user", "Projekt_132!", "projekt_db");

                                $zap = "SELECT nazwa_zad, id_prac, zad_dzien from zadania where id_prac = '$pracownik_aktualny' and zad_dzien='$day'";
                                $result = mysqli_query($conn, $zap) or die("Problemy z odczytem danych!");
                                while($row = mysqli_fetch_assoc($result))
                                {
                                   echo("<center><zad>".$row['nazwa_zad']."<zad></center>"); 
                                }
                            ?>
                        </div>
                    </div>
